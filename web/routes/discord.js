...
router.get("/", (req, res) => {
  ...
  const authorizeUrl = `https://discordapp.com/api/oauth2/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(
    redirectUri
...
router.get("/logout", forceAuth, (req, res) => { req.session.destroy(() => { res.redirect("/") })})
